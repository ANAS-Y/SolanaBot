...
# Get the URL we added to Render
DB_URL = os.getenv("DATABASE_URL")

async def init_db():
    if not DB_URL:
        logging.error("❌ DATABASE_URL is missing! Bot cannot save data.")
        return
    ...
    try:
        ...
        ''')
        await conn.close()
        logging.info("✅ Database connected & tables ready.")
...
async def add_wallet(user_id, pub_key, enc_key, salt):
    conn = await asyncpg.connect(DB_URL)
    try:
        # Postgres 'INSERT ON CONFLICT' handles overwrites securely
        await conn.execute('''
...
async def get_active_trades():
    ...
    try:
        ...
        # Convert Postgres rows to list of tuples to match old bot logic
